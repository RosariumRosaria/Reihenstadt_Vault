cur# SESSION 2 – HEIMKRONE INTEGRATION

---



---

## Kalenderkarte Charter – Queue Scene

#### Environment
- Small administrative office.
- Long queue.
- Motivational posters ("Hang in there!").
- Ticket numbers called.
- Two clerks working slowly.
- Waiting area.
#### Lore Integration
Kalenderkarte:
- Assigned waking hours.
- Assigned work periods.
- Assigned leisure windows.
- Required to carry or sync to COMP/CON.
- Deviation may cause:
  - Disorientation
  - Administrative delay
  - Reassignment of privileges

---

## Queue Friction
#### Couple Argument
Arguing about:
- Transferring to Group Blue.
- Long wait lists.
- Leaving Group Green.
- Family in Green.
- Will claim that Charlie and Mark are a couple.
- Ivan and Simon
Players must intervene to move line. Bribe, help resolve the family dispute 

---

## 3. Journalist Interruption

#### Sleazy local reporter with drone.
Offers:
- Third-in-line spot
- In exchange for info.
- "Abs-so-lute-lee! Names, Greggor, friends call me Greg. I'm with the Krone-Kronciles. Local press group. Mind if we take a few pictures? I'm sure you don't, kind folk like you, offering to help"
#### Steps
- Guesses Group Grey (admin/night side).
- Calls them Lancers.
- Floats rumors:
  - Invasion
  - NHP trap
  - [[Giant squid]]
  - Pirates
- Either way they lose, he gets a headline. Will frame poorly.

---

## 4. Fast-Track Orderly
**Orderly:** "Valued comrades..." Offers fast-tracked integration, a meeting with a representative, and tea. The tea will never arrive. The orderly was held up by a second reporter (similar attire to Greg's associate), who delayed the escort to buy Greg more time with the team.

**Visuals:** The corridor is long and winding, with stark lights. The air is clean, but subtle chemical cleaning products waft through it. Offices and side passages branch off like pulmonary tracks. Coffee machines and printers dot the intersections at small breakrooms like outposts. The front office is merely a face to a far larger organ of the state that huddles behind it. There are dozens of offices before they reach their destination.

**Tea Callback:** If the players return to the Charter later in the campaign, Albert actually serves the tea. Play it straight — he remembers, he apologizes for last time, he has a preferred blend. The callback rewards returning.

---

## 5. Integration Interview Room
Small.
Windowless.
Four chairs.
Old man behind imposing desk.
Motivational posters.
Recording under Statute 36-A.
Albert

#### Procedural Notes
- Verbal intake instead of paper.
- Rerun twice for triplicate compliance.
- Reverse alphabetical order. Ysk, Mark, Indy, Char
- Explicit statute citations.
## Questions
#### All
- Please state the following. 
- Name
- Age
- Home world
- Prior occupation
#### Yskander
- Will be needled for having no home world.
- Will be needled for having narcotics.
- Asked about Heritage authority criminal record.
- Accused of OMNI-Net queries.
- Given child replica Karte.
- Official Karte has additonal Firmware installed for anti-tampering.
#### Mark
- Asked marital status.
- Forced to state or avoid by saying single.
- Monotone condolence response.
#### Indy
- Framed as local overseer of "offworlders".
- UYubtle co-opting attempt.
#### Charlie
- Standard intake.
- Asked regarding cradle and schooling.
- Asked regarding his father. Harold.

---


## 6. Integration Conclusion – Grey Authorization Devices

#### Old Man Closing Statements
- Interview was “only a formality.”
- “Higher powers” already approved them.
- Recording will be transferred to each of them.
- They must:
  - Play it through twice.
  - It will delete afterward.
- No tea after all.
Key line:
> “You’ll have free reign of the station. Departure in two hours.”

The old man slides across three smooth, discreet pieces of technology. Remarkably analog.

**Apperance**:
- Dull silver casing.
- ID number on the bottom.
- OVERSIGHT logo embossed on the back.
- Thin
- A grey rectangle indicator. 

**Instructions**:
- Keep cards near COMP/CON at all times.
- They will sync automatically.
- Additional security firmware has been installed.
- Official Karten remain under Heimkrone authority.
- Replica card given to Yskander as symbolic compliance token.
- Indiana’s device will auto-sync after his next sleep cycle.


